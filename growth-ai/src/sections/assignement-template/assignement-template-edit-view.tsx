...
import { useGetAssignementTemplateById , useUpdateAssignementTemplate } from "@/actions/assignements"
...
export function AssignementTemplateEditView({ assignementId }: AssignmentFormProps) {
    const router = useRouter()
    const { toast } = useToast()
    const { updateAssignementTemplate } = useUpdateAssignementTemplate()

    // Fetch the assignment details
    const { assignementTemplate } = useGetAssignementTemplateById(assignementId)
    ...
    useEffect(() => {
        if (assignementTemplate) {
            form.reset({
                title: assignementTemplate.title,
                description: assignementTemplate.description,
                questions: assignementTemplate.questions.map(q => ({
                    question: q.question,
    ...
    const handleSubmit = async (data: FormValues) => {
        try {
            ...
            if (assignementTemplate?.status) {
                formData.append('status', assignementTemplate.status);
            }
            ...
            await updateAssignementTemplate(formData, assignementId);
            toast({
                title: "Assignment Template Updated",
                description: "Your assignment template has been updated successfully.",
            });
            ...
            toast({
                title: "Error",
                description: "Failed to update assignment Template.",
                variant: "destructive"
            });
        }
    }
